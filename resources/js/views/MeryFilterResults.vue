                    ...
                    <div class="container bar--width">
                        <div class="form-group form-group--custom">
                            <div class="input-group">
                                ...
                                <input class="form-control search-input" v-model="search_name" id="search" placeholder="mery, house heeping …."
                                       type="text">

                                <div class="input-group-prepend prepend-width">
                                    <div class="input-group-text  filter-box" v-bind:class="{'toggle-color': filter_mode}">
                                        <a @click.prevent="filterMode" class="custom-link" id="filter-link">
                                            <img alt="filter" class="filter-icon" src="../assets/img/filter.png" v-if="!filter_mode">
                                            <img alt="filter" class="filter-icon" src="../assets/img/filter-icon.png" v-if="filter_mode">
                ...
                <div class="container job-header bar--width">
                    <div class="row">
                            ...
                            <aside class="filter-options">
                                <form>
                                    <div class="form-group form-select">
                                        ...
                                        <select v-model="country_to_work" class="form-control" id="location" name="country_to_work">
                                            <option v-for="country of countries" :value="country.title">{{ country.title }}</option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group form-select">
                                        <label class="label-text">{{$t("details.salary")}}</label>
                                    ...
                                    <div class="form-group form-select">
                                        <label class="label-text">{{$t("details.type")}}</label>
                                    ...
                                    <button @click.prevent="filter" class="btn btn-red btn-custom" type="submit">{{$t("search")}}</button>
                                    <button @click.prevent="reset" class="btn btn-default btn-custom" type="reset">{{$t("reset")}}</button>
                                </form>
                            </aside>
                        </div>
                        ...
                        <div class=" col-12 mery-wrapper"  v-bind:class="{ 'col-lg-9': filter_mode }">
                            <div class="posts " >
                                <div v-for="mery of merys" class="media job-wrapper " v-bind:class="{'job-width': filter_mode}">
                                    <img width="70"  alt="..." class="mr-3 img-responsive rounded-circle" :src="mery.imgUrl">
                                    <div class="media-body">
                                        <h5 class="job-title"><a class="custom-link">{{ mery.name }}</a>
                                        </h5>
                                        <img alt="place" src="../assets/img/place-2.png"> <span class="job-location">{{ mery.nationality.title }}</span>
                                        <p class="job-duration">
                                            <span class="dur">{{mery.salary}} $ {{$t("job.month")}}</span>
                                        </p>
                                    ...
                                    <div class="align-self-end" v-if="user.role == 3 || user.role == 3">
                                        <router-link :to="{ name: 'public_profile', params:  { id: mery.id }  }" class="more">{{$t("more")}}<img alt="arrow" src="../assets/img/rihgtArrow.png"> </router-link>
                                    </div>
                                </div>
                            </div>
